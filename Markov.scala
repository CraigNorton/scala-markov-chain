...
object MyBuilder extends Builder {
   ...
   */
  private[markov] def buildWithLines(lines: Iterator[String], prefix: Prefix, chain: Chain): Chain = {
    if(!lines.hasNext){
      chain
    }else{
      val theWordsArray = splitWords(lines.next())
      val a = buildWithWords(theWordsArray,prefix,chain)
      buildWithLines(lines,a._1,a._2)
    }
  }
  /**
   ...
   * chain.
   *
   * (2) The recursive step is simple, albeit subtle.
   * We first create a new `Suffix` from the first word in the array of words.
   * Next, we need to add that suffix to the chain. Remember, a `Chain` is a map from `Prefix` to a `Vector[Suffix]`.
   * It is completely reasonable for a `Prefix` to be associated with multiple
   ...
   */
  private[markov] def buildWithWords(words: Array[String], prefix: Prefix, chain: Chain): (Prefix, Chain) =
  {
    if(words.length == 0) {
      return (prefix, chain)
    } else {
      val newSuffix = new Suffix(getFirstWord(words))
      val newPrefix = prefix.shiftIn(getFirstWord(words))
      val newChain = addToChain(chain,prefix,newSuffix)
      buildWithWords(words.tail,newPrefix,newChain)
    }
  }
  /**
...
//  val newSuffix = addSuffix(chain(prefix), suffix)
    if(chain.contains(prefix)){
      val newChain = chain + (prefix -> addSuffix(chain(prefix), suffix))
      newChain
    }else{
      val vex = Vector[Suffix](suffix)
      val newChain = chain + (prefix -> vex)
      newChain
    }
  }
}
...
object MyGenerator extends Generator {
   ...
   */
  def generate(chain: Chain, words: Int, rand: Randomizer): String = generateAll(chain, words, rand, Prefix.empty)
   ...
   * current prefix.
   *
   * We use the length of the vector of suffixes
   * to choose a random suffix from the vector of suffixes.
   *
   * If
   ...
   * in its suffix word.
   *
   * We then append the suffix word, a space,
   * and the return value of the recursive call. We return the
   * resulting string.
   */
  private[markov] def generateAll(chain: Chain, words: Int,rand: Randomizer, prefix: Prefix): String ={
    if(words == 0){
      ""
    }else{
      val vex = getSuffixes(chain, prefix)
      val suf = getRandomSuffix(vex, rand)
      if(isSuffixNewline(suf)){
        ""
      }
      val newPre = prefix.shiftIn(suf.word)
      suf.word + " " + generateAll(chain, words-1, rand, newPre)
    }
  }
  /**
  ...
  private[markov] def getRandomSuffix(suffixes: Vector[Suffix], rand: Randomizer): Suffix =
    {
      suffixes(rand.random(suffixes.length))
    }
